...
def merge_vcf():
    job_memory = "8G"
    if not os.path.isfile("merged_vcf/merged_variants.vcf"):
        statement = '''bcftools merge Variantcalls.dir/*.vcf.gz > merged_vcf/merged_variants.vcf '''
        P.run(statement)
    else:
        pass
...
@follows(merge_vcf,mkdir("correcting_missing_genotype.dir"))

def genotype(): 
    job_memory="6G"
    infile="merged_vcf/merged_variants.vcf"
    outfile="correcting_missing_genotype.dir/merged_variants_corrected.vcf"
...
@follows(genotype,mkdir("MAF_score.dir"))

def MAF ():
    job_memory="6G"
    infile="correcting_missing_genotype.dir/merged_variants_corrected.vcf"
...
@transform(sort, regex(".+/(.+).genome1.star.sorted.bam"),r"Filter_bam.dir/\1.star.filtered.bam")
def filter_bam (infiles,outfile):
    job_memory = "16G"
    infile=infiles
    bam1="sorted_bams.dir/" + os.path.basename(infile).split(".",3)[0] + ".genome1.star.sorted.bam"
    bam2="sorted_bams.dir/" + os.path.basename(infile).split(".",3)[0] + ".genome2.star.sorted.bam"
...
@follows(mkdir("Input_Quasar.dir"))
...
def input_quasar(infiles,outfile):
    ...
    statement = '''python /data/md1srd/Software/pipeline_allele_imbalance/pipeline_split_genome/modified_split_genome_pipeline/base_count.py %(bam)s %(output)s %(fafile)s %(vcf)s'''
    P.run(statement)

@follows(merge_vcf, genotype, MAF, sort, filter_bam, sort_filter,input_quasar)
def full():
    pass
